...
#epoch, batch_size, num_workers, lr(프로세스 수)
...
#user parameter
parser.add_argument('--majority', type=int, default=49, help='majority class count')
parser.add_argument('--minority', type=int, default=5, help='minority class count')
...
class CustomDataset(Dataset):
    def __init__(self, xlsx_path, transforms=None):
        ...
        """
        # Read the csv file
        self.data_info = pd.read_excel(xlsx_path, names=['path', 'label'])
        total_class = 1000
    ...
    def data_processing(self):
        ...
        print('imbalance start')
        
        #데이터 imbalance 작업
        for i in range(0, self.majority_count):
    
            while rand_num in majority_list:
                rand_num = random.randint(1,total_class)
        ...
        for i in range(1, 1001):
            if i not in majority_list:
                minority_list.append(i)
        minority_list.sort()
        
        print('majority: ', majority_list)
        ...
        for major in majority_list:
            ...
            cur_major = []
            for i in range((major-1)*49,(major)*49):
                if self.data_info['label'][i] == major:
                    cur_major.append(i)
            
            live_list = []
            rand_num = random.randint(cur_major[0],cur_major[48])
            ...
            for index in live_list:
                df2.loc[len(df2)] = [self.data_info['path'][index], self.data_info['label'][index]]
        print('majority finish')
        
        for minor in minority_list:
            cur_minor = []
            for i in range((minor-1)*49,(minor)*49):
                if self.data_info['label'][i] == minor:
                    cur_minor.append(i)
                    
            live_list = []
            rand_num = random.randint(cur_minor[0],cur_minor[48])
    ...
    def showinfo(self):
        print('total data count: ', self.data_info['path'].size)
...
def SaveModel(epoch, step):
    print("Save model")
    directory_path = './model_backup/'+str(args.majority)+'_'+str(args.minority)+'_'+str(args.ratio)+'/'
    if not os.path.isdir(directory_path):
        os.mkdir(directory_path)
    save_path = './model_backup/'+str(args.majority)+'_'+str(args.minority)+'_'+str(args.ratio)+'/'+str(epoch).zfill(5)+'_'+str(step).zfill(5)+'.ckpt'
...
if __name__=="__main__":
    ...
    if args.istrain==1:
        ...
        train_set = CustomDataset('./temp_.xlsx',transform)
        ...
        test_loader = torch.utils.data.DataLoader(test_set, 
                                           ...
                                           shuffle=False, 
                                           num_workers=args.workers)
        load_path = './model_backup/'+str(args.majority)+'_'+str(args.minority)+'_'+str(args.ratio)
        ...
        print(highest_model_p)
        print(highest_model_accuracy)
        print()
        
        high_save_path = './model_backup/'+str(args.majority)+'_'+str(args.minority)+'_'+str(args.ratio)+'/highest'+'/'
        if not os.path.isdir(high_save_path):
            os.mkdir(high_save_path)
        file_path = load_path+'/'+highest_model_p
